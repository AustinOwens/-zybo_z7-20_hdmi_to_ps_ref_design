...
class MMIOShell(MMIO, Cmd):

    intro = "Memory mapping tool. Type help or ? to list commands.\n"                                                                      
    prompt = "MMIO$ "   

    def __init__(self, physaddr, size):

        MMIO.__init__(self, physaddr, size)
        Cmd.__init__(self)
        
    def do_read32(self, arg):
        """
        read32 offset
        """
        print self.read32(int(arg))

    def do_read16(self, arg):
        """
        read16 offset
        """
        print self.read16(int(arg)) 

    def do_read8(self, arg):
        """
        read8 offset
        """
        print self.read8(int(arg))
    ...
    def do_read(self, arg):
        ...
        """
        args = arg.split(" ")
        reg_dump = self.read(int(args[0]), int(args[1]))
        
        print_byte_list = []
        for b in reg_dump:
            print_byte_list.append(b)
            if len(print_byte_list) >= 16:
                print " ".join("0x{:02X}".format(ord(b)) for b in print_byte_list)
                print_byte_list = []
        
        if len(print_byte_list) > 0:
            print " ".join("0x{:02X}".format(ord(b)) for b in print_byte_list)


    def do_write32(self, arg):
        """
        write32 offset value
        """
        args = arg.split(" ")
        self.write32(int(args[0]), int(args[1]))
    ...
    def do_write16(self, arg):
        ...
        """
        args = arg.split(" ")
        self.write16(int(args[0]), int(args[1]))
                                                                                                                                           
    def do_write8(self, arg):
        """
        write8 offset value
        """
        args = arg.split(" ")
        self.write8(int(args[0]), int(args[1]))
    ...
    def do_write(self, arg):
        ...
        """
        args = arg.split(" ")
        self.write(int(args[0]), int(args[1]))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Maps physical memory to virtual memory. Enter a base physical address and the memory region size.")
    parser.add_argument('physaddr', metavar='physaddr', type=auto_int, help='Base physical address')
    parser.add_argument('size', metavar='size', type=auto_int, help='Memory region size')

    args = parser.parse_args()
    
    MMIOShell(**vars(args)).cmdloop()
